...
void ggml_cuda_opt_step_adamw(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    float       * src0_d         = (float       *)GGML_CUDA_NAME_TENSOR(src0->data, src0);
    const float * src0_grad_d    = (const float *)GGML_CUDA_NAME_TENSOR(src0_grad->data, src0_grad);
    float       * src0_grad_m_d  = (float       *)GGML_CUDA_NAME_TENSOR(src0_grad_m->data, src0_grad_m);
    float       * src0_grad_v_d  = (float       *)GGML_CUDA_NAME_TENSOR(src0_grad_v->data, src0_grad_v);
    const float * adamw_params_d = (const float *)GGML_CUDA_NAME_TENSOR(adamw_params->data, adamw_params);
