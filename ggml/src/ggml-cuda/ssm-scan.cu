...
void ggml_cuda_op_ssm_scan(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const float * src2_d = (const float *) GGML_CUDA_NAME_TENSOR(src2->data, src2);
    const float * src3_d = (const float *) GGML_CUDA_NAME_TENSOR(src3->data, src3);
    const float * src4_d = (const float *) GGML_CUDA_NAME_TENSOR(src4->data, src4);
    const float * src5_d = (const float *) GGML_CUDA_NAME_TENSOR(src5->data, src5);
    const int32_t * src6_d = (const int32_t *) GGML_CUDA_NAME_TENSOR(src6->data, src6);
