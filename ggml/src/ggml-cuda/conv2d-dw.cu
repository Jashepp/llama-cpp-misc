...
void ggml_cuda_op_conv2d_dw(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const float * w_d = (const float *)GGML_CUDA_NAME_TENSOR(kernel->data, kernel);
    const float * x_d = (const float *)GGML_CUDA_NAME_TENSOR(input->data, input);
    float *       y_d = (float *)GGML_CUDA_NAME_TENSOR(dst->data, dst);
