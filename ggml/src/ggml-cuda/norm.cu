...
void ggml_cuda_op_rms_norm_fused(ggml_backend_cuda_context & ctx, ggml_tensor * dst, ggml_tensor * mul_tensor) {
    ...
    const float * src0_d = (const float *) GGML_CUDA_NAME_TENSOR(rms_norm_src->data, rms_norm_src);
    const float * mul_d = nullptr;
    const ggml_tensor * mul_src = nullptr;

    if (mul_tensor->src[0] == dst) {
        mul_d = (float *) GGML_CUDA_NAME_TENSOR(mul_tensor->src[1]->data, mul_tensor->src[1]);
        mul_src = mul_tensor->src[1];
    } else if(mul_tensor->src[1] == dst) {
        mul_d = (float *) GGML_CUDA_NAME_TENSOR(mul_tensor->src[0]->data, mul_tensor->src[0]);
        mul_src = mul_tensor->src[0];
    } else {
        GGML_ASSERT(false);
    }

    float * dst_d = (float *) GGML_CUDA_NAME_TENSOR(mul_tensor->data, mul_tensor);
    cudaStream_t stream = ctx.stream();
...
void ggml_cuda_op_rms_norm_fused_add(ggml_backend_cuda_context & ctx,
    ...
    if (add_tensor->src[0] == mul_tensor) {
        add_d   = (float *) GGML_CUDA_NAME_TENSOR(add_tensor->src[1]->data, add_tensor->src[1]);
        add_src = add_tensor->src[1];
    } else if (add_tensor->src[1] == mul_tensor) {
        add_d   = (float *) GGML_CUDA_NAME_TENSOR(add_tensor->src[0]->data, add_tensor->src[0]);
        add_src = add_tensor->src[0];
    } else {
        GGML_ASSERT(false);
    }

    float *      dst_d  = (float *) GGML_CUDA_NAME_TENSOR(add_tensor->data, add_tensor);
    cudaStream_t stream = ctx.stream();
...
void ggml_cuda_op_rms_norm_back(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const float * grad_d  = (const float *) GGML_CUDA_NAME_TENSOR(grad->data, grad);
    const float * src0f_d = (const float *) GGML_CUDA_NAME_TENSOR(src0f->data, src0f);
    float       * dst_d   = (float       *) GGML_CUDA_NAME_TENSOR(dst->data, dst);
