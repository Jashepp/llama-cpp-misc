...
void ggml_cuda_count_equal(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    int64_t * dst_d  = (int64_t *)GGML_CUDA_NAME_TENSOR(dst->data, dst);
    ...
    switch (src0->type) {
        case GGML_TYPE_I32: {
            const int * src0_d = (const int *)GGML_CUDA_NAME_TENSOR(src0->data, src0);
            const int * src1_d = (const int *)GGML_CUDA_NAME_TENSOR(src1->data, src1);
