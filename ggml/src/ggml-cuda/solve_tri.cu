...
void ggml_cuda_op_solve_tri(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (n <= MAX_N_FAST && k <= MAX_K_FAST) {
        solve_tri_f32_cuda((const float *)GGML_CUDA_NAME_TENSOR(src0->data, src0), (const float *)GGML_CUDA_NAME_TENSOR(src1->data, src1), (float *)GGML_CUDA_NAME_TENSOR(dst->data, dst), n, k,
                           src0->ne[2], src0->ne[3], src0->nb[2] / sizeof(float), src0->nb[3] / sizeof(float),
                           src1->nb[2] / sizeof(float), src1->nb[3] / sizeof(float), dst->nb[2] / sizeof(float),
                           dst->nb[3] / sizeof(float), ctx.stream());
    } else {
        solve_tri_f32_cublas(ctx, (const float *)GGML_CUDA_NAME_TENSOR(src0->data, src0), (const float *)GGML_CUDA_NAME_TENSOR(src1->data, src1), (float *)GGML_CUDA_NAME_TENSOR(dst->data, dst), n, k,
